...
**Unlock-mode grid and snapping: Libs/LibSquizzGrid-1.0**, shared with SquizzFrames (a copy in each;
LibStub runs the newest). ⚠ Change BOTH copies identically and bump `MINOR`, or one addon silently
runs the other's copy. On-screen frames drag through **`BH:GridStartMoving(frame)` /
`BH:GridStopMoving(frame)`, never `frame:StartMoving()`**: with Snap off (the default) that is the
same StartMoving; with it on, a snapping drag that leaves the frame anchored CENTER to UIParent's
CENTER, which both savers read correctly (`SaveFramePos` stores `GetPoint()` generically, CDM groups
use `GetCenter()`). Windows (options panel, dialogs, the floating unlock control) deliberately keep
the plain drag. Snap targets: `POSITION_PAIRS` frames + main/callouts/lust frames are registered on
entering unlock mode, CDM containers at creation, and anything dragged registers itself. Settings:
`SquizzumablesDB.snapGrid`.
